...
export default function LancarNotasScreen({ route, navigation }) {
  // Captura o professorId passado pela navegação (se houver)
  ...
  const carregarDados = async () => {
    setLoading(true);
    try {
      // Busca dados. Se houver professorId, a API deve filtrar as disciplinas no back-end
      const [dadosAlunos, dadosDisciplinas] = await Promise.all([
        apiGetAlunos(),
        apiGetDisciplinas(professorId) // Passamos o professorId para filtrar
      ]);
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      Alert.alert('Sucesso', 'Registro acadêmico atualizado com sucesso!');
      navigation.goBack();
    } catch (err) {
        ...
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.horizontalScroll}>
            {alunos.map(aluno => (
              ...
              >
                <Text style={[styles.chipText, form.aluno_id === aluno.id && styles.chipTextSelected]}>{aluno.nome}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.horizontalScroll}>
            {disciplinas.length > 0 ? disciplinas.map(disc => (
              <TouchableOpacity 
                key={disc.id} 
                style={[styles.chip, form.disciplina_id === disc.id && styles.chipSelected]}
                onPress={() => setForm({ ...form, disciplina_id: disc.id })}
              >
                <Text style={[styles.chipText, form.disciplina_id === disc.id && styles.chipTextSelected]}>{disc.nome}</Text>
              </TouchableOpacity>
            )) : <Text style={{color: colors.muted}}>Nenhuma disciplina vinculada encontrada.</Text>}
          </ScrollView>

          <Text style={styles.sectionTitle}>3. REGISTRO ACADÊMICO</Text>
          <View style={styles.row}>
            <View style={{ flex: 1, marginRight: 10 }}>
              <AppInput label="Nota 1º Bim" value={form.nota1} onChangeText={t => setForm({...form, nota1: t.replace(/[^0-9.]/g, '')})} keyboardType="numeric" />
            </View>
            <View style={{ flex: 1, marginRight: 10 }}>
              <AppInput label="Nota 2º Bim" value={form.nota2} onChangeText={t => setForm({...form, nota2: t.replace(/[^0-9.]/g, '')})} keyboardType="numeric" />
            </View>
            <View style={{ flex: 1 }}>
              <AppInput label="Faltas" value={form.faltas} onChangeText={t => setForm({...form, faltas: t.replace(/[^0-9]/g, '')})} keyboardType="numeric" />
            </View>
          </View>
          ...
          <View style={styles.tagsContainer}>
            {tiposFalta.map((tipo, index) => (
              <TouchableOpacity key={index} style={[styles.tag, form.tipo_falta === tipo && styles.tagSelected]} onPress={() => setForm({ ...form, tipo_falta: tipo })}>
                <Text style={[styles.tagText, form.tipo_falta === tipo && styles.tagTextSelected]}>{tipo}</Text>
              </TouchableOpacity>
            ))}
          </View>

          <AppButton title={saving ? "Salvando..." : "Gravar no Boletim"} onPress={handleSalvar} loading={saving} style={{ marginTop: 40 }} />
        </ScrollView>
