...
import { apiCriarProfessor } from '../services/api';
...
const INITIAL = { nome: '', titulacao: '', area: '', tempo_docencia: '', email: '' };
...
export default function CadastroProfessorScreen({ navigation }) {
  ...
  function validate() {
    const errs = {};
    ['nome', 'titulacao', 'area', 'tempo_docencia', 'email'].forEach(f => {
  ...
  async function handleSave() {
    ...
    setLoading(true);
    try {
      await apiCriarProfessor(form);
      Alert.alert('Sucesso! ✅', `Professor(a) "${form.nome}" cadastrado(a).`, [
        { text: 'OK', onPress: () => navigation.goBack() },
      ]);
    } catch (err) {
      const msg = err.response?.data?.erro || 'Erro ao cadastrar professor.';
      ...
      <View style={{ gap: 5 }}>
        ...
        {open && (
          <View style={pickerStyle.dropdown}>
            {options.map(opt => (
              <TouchableOpacity key={opt} style={pickerStyle.option} onPress={() => { onSelect(opt); setOpen(false); }}>
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <FormHeader title="Cadastro de Professor" subtitle="Dados do docente" onBack={() => navigation.goBack()} />
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">

          <SectionLabel>👨‍🏫 Informações Profissionais</SectionLabel>

          <AppInput label="Nome Completo *" value={form.nome} onChangeText={set('nome')} placeholder="Ex: André Olímpio" error={errors.nome} />

          <PickerField label="Titulação *" value={form.titulacao} options={TITULACOES}
            open={showTitulacao} setOpen={setShowTitulacao} onSelect={set('titulacao')} error={errors.titulacao} />

          <AppInput label="Área de Atuação *" value={form.area} onChangeText={set('area')} placeholder="Ex: Desenvolvimento Mobile" error={errors.area} />

          <PickerField label="Tempo de Docência *" value={form.tempo_docencia} options={TEMPOS}
            open={showTempo} setOpen={setShowTempo} onSelect={set('tempo_docencia')} error={errors.tempo_docencia} />

          <AppInput label="E-mail *" value={form.email} onChangeText={set('email')} placeholder="professor@fatec.sp.gov.br" keyboardType="email-address" error={errors.email} />
